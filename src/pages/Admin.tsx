...
type Country = { id:number; name:string; iso2:string; currency:string; regulator_name:string; enabled:number; bank_count:number; source_count:number; report_count:number; last_scan_at?:string|null };
...
function Console({token,logout}:{token:string;logout:()=>void}){
 ...
 async function analyze(id:number){
   setBusy(id); setProgress(p=>({...p,[id]:"Starting analysis…"}));
   try{
     ...
     while(true){
       ...
       for(const line of lines){
         ...
         try{
           ...
           if(x.status==="checking") setProgress(p=>({...p,[id]:`Checking portal ${x.checked}/${x.total} · ${x.newReports||0} new reports · ${x.published||0} values published`}));
           else if(x.status==="source_done") setProgress(p=>({...p,[id]:`Portal complete · ${x.reportsFound||0} reports found · ${x.newReports||0} new · ${x.published||0} values published`}));
           else if(x.status==="done") setProgress(p=>({...p,[id]:`Complete · ${x.result.newReports} new reports · ${x.result.published} values published`}));
 ...
 <div className="admin-stats"><div><b>{countries.length}</b><span>Countries</span></div><div><b>{countries.reduce((n,c)=>n+c.bank_count,0)}</b><span>Banks configured</span></div><div><b>{countries.reduce((n,c)=>n+c.source_count,0)}</b><span>Report portals</span></div><div><b>{countries.reduce((n,c)=>n+c.report_count,0)}</b><span>Reports processed</span></div></div>
...
function CountryConfig({token,config,onClose,reload}:{token:string;config:any;onClose:()=>void;reload:()=>void}){
 const [bank,setBank]=useState({name:"",sourceUrl:""});const [sourceBank,setSourceBank]=useState("");const [sourceUrl,setSourceUrl]=useState("");
